...
set "TMPFILE=%TEMP%\streamlit_%RANDOM%.py"

(
    for /f "tokens=1,* delims=:" %%A in ('findstr /n "^" "%~f0"') do (
        if "%%B"=="#PYTHON_START" set START=1
        if defined START if not "%%B"=="#PYTHON_START" echo.%%B
    )
) > "%TMPFILE%"

streamlit run "%TMPFILE%" -- %*
del "%TMPFILE%" >nul 2>&1
exit /b
...
st.altair_chart(cases_chart, width="stretch")
